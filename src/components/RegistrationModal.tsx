...
export const RegistrationModal: React.FC<RegistrationModalProps> = ({ onClose, onSwitchToLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // 1. ADIM: Jira doğrulama (Edge Function üzerinden)
    setLoadingStep('validating');
    try {
      const jiraRes = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/validate-jira`,
        {
          method: 'POST',
          headers: {
      ...
      if (!jiraRes.ok) {
        ...
        setValidationError({
          title: data.title || 'Bağlantı hatası',
          description: data.description || 'Jira doğrulaması başarısız.',
          hint: data.hint,
          hintLink: data.hintLink,
        });
        setLoading(false);
        setLoadingStep('idle');
        return;
