...
export const TaskList: React.FC<TaskListProps> = ({ tasks, onEditTask, onDeleteTask, onAddTask }) => {
  ...
  const filteredTasks = tasks.filter(task => {
    ...
    const matchesBank = bankFilter === 'all' || task.bank === bankFilter;
                ...
                >
                  <td className="px-6 py-4">
                    <div className="flex items-start space-x-3">
                      <span className="text-lg">{getPriorityIcon(task.status)}</span>
          ...
          <div className="flex items-center justify-between text-sm text-gray-600">
            ...
            <div className="flex items-center space-x-4">
              <span className="flex items-center space-x-1">
                ...
                <span>{sortedTasks.filter(t => t.status === 'todo').length} Bekliyor</span>
              </span>
              <span className="flex items-center space-x-1">
                <span className="w-2 h-2 bg-blue-500 rounded-full"></span>
                <span>{sortedTasks.filter(t => t.status === 'in-progress').length} Devam</span>
              </span>
              <span className="flex items-center space-x-1">
                <span className="w-2 h-2 bg-green-500 rounded-full"></span>
                <span>{sortedTasks.filter(t => t.status === 'done').length} Tamam</span>
              </span>
            </div>
          </div>
        </div>
