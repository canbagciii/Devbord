  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
          ...
          <div className="space-y-3">
            ...
            {bankName === 'Tüm Bankalar' && (
              <div className="flex justify-between">
                <span className="text-gray-600 dark:text-gray-400">Aktif Banka:</span>
                <span className="font-medium text-gray-900 dark:text-gray-100">{uniqueBanks.length}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Toplam Görev:</span>
              <span className="font-medium text-gray-900 dark:text-gray-100">{tasks.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Tamamlanan:</span>
              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                doneTasks.length > 0 ? 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300' : 'bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-300'
              }`}>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-200 dark:border-gray-700">
          <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 mb-4">Kaynak Dağılımı</h3>
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Aktif Yazılımcı:</span>
              <span className="font-medium text-gray-900 dark:text-gray-100">{uniqueDevelopers.length}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Analist Tahmini Süre:</span>
              <span className="font-medium text-gray-900 dark:text-gray-100">{totalEstimatedHours} saat</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600 dark:text-gray-400">Gerçekleşen:</span>
              <span className="font-medium text-gray-900 dark:text-gray-100">{totalActualHours} saat</span>
            </div>
          </div>
        </div>
      </div>
