...
import KolayIKOnboarding, { useKolayIKOnboarding } from './KolayIKEmployeesOnboarding';

export const KolayIKEmployees: React.FC = () => {
  const { hasRole } = useAuth();
  const { isOnboardingOpen, openOnboarding, closeOnboarding } = useKolayIKOnboarding();
  ...
  const departments = Array.from(new Set(
    employeesWithLeave.map(emp => emp.department).filter(Boolean).map(dept => dept!)
  )).sort();
  ...
  const exportEmployeesToCSV = () => {
    if (filteredEmployees.length === 0) { alert('İndirilecek çalışan verisi bulunamadı.'); return; }
    const csvHeaders = ['ID', 'Ad', 'Soyad', 'Tam Ad', 'E-posta', 'Departman', 'Pozisyon', 'Durum', 'İşe Başlama Tarihi'];
    const csvData = filteredEmployees.map(emp => [
      emp.id, emp.firstName, emp.lastName, emp.fullName, emp.email,
  ...
  const exportLeaveDataToCSV = () => {
    if (leaveData.length === 0) { alert('İndirilecek izin verisi bulunamadı.'); return; }
    const csvHeaders = ['Yazılımcı', 'E-posta', 'Toplam İzin Günü', 'İzin Detayları', 'Ay'];
    const csvData = leaveData.map(dev => [
      dev.developerName, dev.email, dev.leaveDays,
      dev.leaveDetails.map(leave => `${leave.leaveType}: ${leave.startDate} - ${leave.endDate} (${leave.days} gün)`).join('; '),
      selectedMonth
    ]);
    const csvContent = [csvHeaders.join(','), ...csvData.map(row => row.map(cell => `"${String(cell).replace(/"/g, '""')}"`).join(','))].join('\n');
    const BOM = '\uFEFF';
  ...
  <div className="flex items-center space-x-3">
    ...
    */}
    ...
    >
      ...
      {/* Connection Status */}
      {connectionStatus && (
        <div className={`rounded-lg p-4 border ${connectionStatus.success ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
          <div className="flex items-center space-x-2">
            ...
            <p className={connectionStatus.success ? 'text-green-800' : 'text-red-800'}>{connectionStatus.message}</p>
          </div>
        </div>
      )}
      ...
      {error && (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <button onClick={refresh} className="mt-3 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors">
            Tekrar Dene
          </button>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex items-center justify-between">
            <div><p className="text-sm font-medium text-gray-600">Toplam Çalışan</p><p className="text-2xl font-bold text-blue-600">{stats.total}</p></div>
            <Users className="h-8 w-8 text-blue-600" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex items-center justify-between">
            <div><p className="text-sm font-medium text-gray-600">Aktif</p><p className="text-2xl font-bold text-green-600">{stats.active}</p></div>
            <CheckCircle className="h-8 w-8 text-green-600" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex items-center justify-between">
            <div><p className="text-sm font-medium text-gray-600">Pasif</p><p className="text-2xl font-bold text-red-600">{stats.inactive}</p></div>
            <XCircle className="h-8 w-8 text-red-600" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex items-center justify-between">
            <div><p className="text-sm font-medium text-gray-600">E-posta Var</p><p className="text-2xl font-bold text-purple-600">{stats.withEmail}</p></div>
            <Mail className="h-8 w-8 text-purple-600" />
          </div>
        </div>
        <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
          <div className="flex items-center justify-between">
            <div><p className="text-sm font-medium text-gray-600">Departman</p><p className="text-2xl font-bold text-orange-600">{stats.departments}</p></div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-4 border border-gray-200">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <select value={departmentFilter} onChange={(e) => setDepartmentFilter(e.target.value)} className="border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent">
            <option value="all">Tüm Departmanlar</option>
            {departments.map(dept => <option key={dept} value={dept}>{dept}</option>)}
          </select>
        </div>
        {searchTerm && <div className="mt-2 text-sm text-gray-600">{filteredEmployees.length} çalışan gösteriliyor</div>}
      </div>
      ...
      {!loading && filteredEmployees.length > 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-200">
            ...
            <p className="text-sm text-gray-600 mt-1">{selectedMonth} ayında izni olan {filteredEmployees.length} çalışan</p>
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">İzin Detayları</th>
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Kapasite Etkisi</th>
                  {showDetails && (
                    <>
                      ...
                      <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">İşe Başlama</th>
                    </>
                  )}
                  <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Durum</th>
                  {showDetails && <th className="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">ID</th>}
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={employee.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/30'}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-3">
                          <div className="flex-shrink-0 h-10 w-10 bg-blue-100 rounded-full flex items-center justify-center">
                            <span className="text-sm font-medium text-blue-800">{employee.firstName[0]}{employee.lastName[0]}</span>
                          </div>
                          <div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center space-x-2">
                          {employee.email ? (
                            <><Mail className="h-4 w-4 text-gray-400" /><span className="text-sm text-gray-900">{employee.email}</span></>
                          ) : (
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center space-x-1">
                          ...
                          <span className={`text-sm font-medium ${employeeLeaveData && employeeLeaveData.leaveDays > 0 ? 'text-orange-600' : 'text-gray-400'}`}>
                            ...
                            {employeeLeaveData.leaveDetails.map((leave, idx) => {
                              ...
                              return (
                                <div key={idx} className={`text-xs rounded px-2 py-1 ${isPublicHoliday ? 'bg-purple-50 border border-purple-200' : 'bg-orange-50 border border-orange-200'}`}>
                                  ...
                                  {leave.description && <div className={`italic ${isPublicHoliday ? 'text-purple-600' : 'text-orange-600'}`}>{leave.description}</div>}
                                </div>
                              );
                            })}
                          </div>
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center space-x-1">
                          ...
                          <span className={`text-sm font-medium ${employeeLeaveData && employeeLeaveData.leaveDays > 0 ? 'text-red-600' : 'text-gray-400'}`}>
                            -{(employeeLeaveData?.leaveDays || 0) * 7}h
                          </span>
                        </div>
                        {employeeLeaveData && employeeLeaveData.leaveDays > 0 && (
                          <div className="text-xs text-gray-500 mt-1">70h → {70 - (employeeLeaveData.leaveDays * 7)}h</div>
                        )}
                      </td>
                      {showDetails && (
                        <>
                          <td className="px-6 py-4">
                            <div className="flex items-center space-x-2">
                              {employee.department ? (
                                <><Building className="h-4 w-4 text-gray-400" /><span className="text-sm text-gray-900">{employee.department}</span></>
                              ) : (
                                <span className="text-sm text-gray-400 italic">Departman yok</span>
                              )}
                            </div>
                          </td>
                          <td className="px-6 py-4"><span className="text-sm text-gray-900">{employee.position || '-'}</span></td>
                          <td className="px-6 py-4 text-center">
                            <div className="flex items-center justify-center space-x-2">
                              {employee.startDate ? (
                                <><Calendar className="h-4 w-4 text-gray-400" /><span className="text-sm text-gray-900">{new Date(employee.startDate).toLocaleDateString('tr-TR')}</span></>
                              ) : (
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-2 py-1 text-xs font-medium rounded-full border ${employee.isActive ? 'bg-green-100 text-green-800 border-green-200' : 'bg-red-100 text-red-800 border-red-200'}`}>
                          {employee.isActive ? 'Aktif' : 'Pasif'}
                        </span>
                      </td>
                      {showDetails && (
                        <td className="px-6 py-4 text-center"><span className="text-sm font-mono text-gray-600">{employee.id}</span></td>
                      )}
                    </tr>
                  );
                })}
          ...
          <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
            <div className="flex items-center justify-between text-sm">
              <span className="text-gray-600">{selectedMonth} ayında izni olan {filteredEmployees.length} çalışan gösteriliyor</span>
              <div className="flex items-center space-x-4">
                <span className="flex items-center space-x-1"><span className="w-2 h-2 bg-green-500 rounded-full"></span><span>{stats.active} Aktif</span></span>
                <span className="flex items-center space-x-1"><span className="w-2 h-2 bg-red-500 rounded-full"></span><span>{stats.inactive} Pasif</span></span>
              </div>
            </div>
          </div>
        </div>
      ...
      {!loading && employeesWithLeave.length === 0 && !error && (
        <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
          ...
          <p className="text-gray-400 text-sm mt-2">Farklı bir ay seçin veya API bağlantısını kontrol edin.</p>
        </div>
      )}
            ...
            {departments.map(dept => {
              ...
              return (
                <div key={dept} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between text-sm"><span className="text-gray-600">Toplam:</span><span className="font-medium">{deptEmployees.length}</span></div>
                    <div className="flex justify-between text-sm"><span className="text-gray-600">Aktif:</span><span className="font-medium text-green-600">{activeDeptEmployees.length}</span></div>
                    <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                      <div className="bg-green-500 h-2 rounded-full" style={{ width: `${(activeDeptEmployees.length / deptEmployees.length) * 100}%` }} />
                    </div>
                  </div>
                </div>
              );
